...
public class GeneticAlgorithm {
    ...
    protected static int calculateFitness(Individual individual){
        ...
        for(int i = 0; i < individual.getLength() && i < getLength(); i++){
            //System.out.println(individual.getGene(i) + "*" + "2^" + i + "=" + " = " + solution[i]*Math.pow(2, i));
            fitness += individual.getGene(i) * Math.pow(2, i);
        }
        return (int) Math.pow(fitness, 2);
    }
    ...
    public int getSolutionFitness(){
        ...
        for(int i = 0; i < solution.length; i++){
            fitness += solution[i] * Math.pow(2, i);
        }
        return (int) Math.pow(fitness, 2);
    }
}
